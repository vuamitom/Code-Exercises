...
class robot:
    ...
    def move(self, motions):
        a, d = motions
        a = random.gauss(a, self.steering_noise)
    ...
    def sense(self, use_noise = 1):
        ...
        for l in landmarks:
            y, x = l
            bear = atan2(y - self.y, x - self.x) - self.orientation
            bear = bear if bear >= 0 else 2*pi + bear 
            bear = bear % (2 *pi)
            if use_noise:
                bear = random.gauss(bear, self.bearing_noise)
            Z.append(bear)
        return Z
...
e = particle_filter(motions, measurements)
fr = robot(0)
fr.set(93.476, 75.186, 5.2664)

print check_output(fr, e)
