package com.vng.zing.experiment.brics;
...
final public class StringUnionOperations {
	 ...
	 */
	private static com.vng.zing.experiment.brics.State convert(State s,
			IdentityHashMap<State, com.vng.zing.experiment.brics.State> visited) {
		com.vng.zing.experiment.brics.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new com.vng.zing.experiment.brics.State();
	 ...
	 */
	public static com.vng.zing.experiment.brics.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations(); 

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, com.vng.zing.experiment.brics.State>());
	}
