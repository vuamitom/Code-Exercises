...
def verify_balance_across_class(train_datasets):
  meta = []
  for fl in train_datasets:
    with open(fl, 'rb') as f:
      dataset = pickle.load(f)
      meta.append(dataset.shape[0])
  print (meta)
  print ('mean: ', np.mean(meta))
  print ('std dev: ', np.std(meta))

# verify_balance_across_class(train_datasets)
...
def validate_data_after_shuffling(dataset, labels):
  idx = np.random.randint(len(labels), size=1)[0]
  d, l = dataset[idx], labels[idx]
  print ('showing letter for label ', l)
  plt.imshow(d)
  plt.show()
# validate_data_after_shuffling(train_dataset, train_labels)
